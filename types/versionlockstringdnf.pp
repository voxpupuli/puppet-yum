...
# `%{NAME}-%{EPOCH}:%{VERSION}-%{RELEASE}.%{ARCH}`.  As a Regex, it
...
# yum-versionlock.
#
# @example A complete, well-formed string: `bash-0:4.1.2-9.el6_2.x86_64'
# @example A well-formed string that has dropped the optional BuildArch sub-field: `bash-0:4.1.2-9.el6_2`
# @example A well-formed string using wildcards: `bash*-*0:4.*-*.*`
# @example An invalid string (wildcard spans the VERSION and RELEASE fields): `bash-0:4.*-el6.x86_64
# @example An invlaid string (wildcard spans the VERSION, RELEASE, and ARCH fields): `bash-0:*`
#
type Yum::VersionlockStringDNF = Pattern[/^([0-9a-zA-Z\._\+%\{\}\*-]+)-([0-9\*]+):([^-]+)-([^-]+)\.(([0-9a-zZ-Z_\*]+)(?:\.(noarch|x86_64|i386|arm|ppc64|ppc64le|sparc64|ia64|alpha|ip|m68k|mips|mipsel|mk68k|mint|ppc|rs6000|s390|s390x|sh|sparc|xtensa|\*))?)$/]
